...
def build_block(base_offset , string_list , jp_string_list):
    buffer = StringIO()
    buffer.write("TXT2")
    ...
    for i in xrange(len(string_list)):
        ...
        jpstring = jp_string_list[i]
        if not string.count("}") == jpstring.count("}"):
            print(u"error:ctr code count error.")
            print("---------------------------")
            print("%s"%jpstring.replace(u"\u30fb" ,u"."))
            print("+++++++++++++++++++++++++++")
            print("%s"%string)
            print("---------------------------")
...
def repack_text(fn):
    ...
    o_buffer = fp.read()
    fp.seek(0)
    cnlines = codecs.open("jpTEXT//%s.txt"%fn , "rb" , "utf-16").readlines()
    jplines = codecs.open("jpTEXT//%s.txt"%fn , "rb" , "utf-16").readlines()
    string_list = makestr(cnlines)
    jp_string_list = makestr(jplines)
    dest = StringIO()
    dest.write(o_buffer)
    dest.seek(0)
    ...
    if currentPos > 0:
        ...
        dest.truncate()
        block_data = build_block(currentPos , string_list , jp_string_list)
